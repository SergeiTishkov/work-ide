...
def test_merge_vacancy_preserves_external_signals_on_rerun(isolated_data_dir):
    ...
    # — external_signals (a pay estimate found by hand) must not be lost, any
    # more than manual is.
    vacancies = {}
...
def test_merge_vacancy_preserves_manual_edits_on_rerun(isolated_data_dir):
    ...
    v = vacancies["vac-1"]
    assert v["manual"]["status"] == "applied", "a status set by the owner must not be overwritten by a repeat run"
