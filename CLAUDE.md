...
> Кодовое имя проекта: **Work IDE** — второй рабочий стол, за которым сидит тот же
> инженер, но работает вполсилы и получает вторую зарплату.
>
...
> `kisel_criteria.yaml` физически не может повлиять на другую идентичность.
...
1. **Гео-правила и языковые фильтры выводятся, а не копируются.** Они —
   ...
   резидента Грузии дисквалификация, для резидента США — плюс. Таблицы вывода:
   `config/derivation/`.
...
- **Парсеры — да. Обход защиты — нет.** Граница проходит здесь, и она
  ...
  - **Делаем свободно**: парсеры любых источников, которые отдают данные
    ...
    скорее на стороне парсинга. Владелец, кроме того, резидент
    юрисдикции, где это тем более не преступление.
...
4. Что ближе к профилю "скучная легаси удалёнка из США/Канады, низкая
   нагрузка, высокая оплата, низкая конкуренция"?
